...
public class xiaoQiu extends Activity
{
    // 桌面的宽度
    ...
    // 小球横向的运行速度
    private float xSpeed = (int) (ySpeed * xyRate * 2);
    // ballX和ballY代表小球的座标
    ...
    public void onCreate(Bundle savedInstanceState)
    {
        ...
        timer.schedule(new TimerTask() //
        {
            ...
            public void run()
            {
                ...
                if (ballX <= BALL_SIZE)
                {
                    xSpeed =   Math.abs((float)(100*(Math.random()+1)));
                }
                else if(ballX >= tableWidth - BALL_SIZE){
                    xSpeed =  - Math.abs((float)(100*(Math.random()+1)));
                }
